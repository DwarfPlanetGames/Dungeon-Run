...
public class LevelScreen implements Screen {
	...
	public void render(float delta) {
		...
		if (texX < -tex.getWidth() + Gdx.graphics.getWidth())
			texX = -tex.getWidth() + Gdx.graphics.getWidth();
		if (texX > 0)
			texX = 0;
		if (texY < -tex.getHeight() + Gdx.graphics.getHeight())
			texY = -tex.getHeight() + Gdx.graphics.getHeight();
		if (texY > 0)
			texY = 0;
		...
		if (isTouching && !dragging) {
			...
			batch.draw(PlayScreen.gradient,
					(int) ((Gdx.input.getX() - texX) / 100f) * 100 + texX,
					texY, 0, 0, 1, 1, 100, tex.getHeight(), 0);
		}
		...
		for (int i = 0; i < 30; i++) {
			if (i <= maxLev) {
				...
				batch.draw(gradient, i * 100 + texX, 0, 100,
						Gdx.graphics.getHeight());
			} else {
				PlayScreen.gradient.setRegion(255, 0, 1, 1);
				batch.draw(PlayScreen.gradient, i * 100 + texX, texY, 0, 0, 1,
						1, 100, tex.getHeight(), 0);
			}
		}
		...
		if (Gdx.input.isTouched()) {
			...
			int thresh = 5;
			if (Gdx.input.getX() < X - thresh || Gdx.input.getX() > X + thresh
					|| Gdx.input.getY() < Y - thresh
					|| Gdx.input.getY() > Y + thresh) {
